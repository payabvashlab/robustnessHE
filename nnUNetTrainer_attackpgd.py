...
class nnUNetTrainer(object):
    ...
    def set_deep_supervision_enabled(self, enabled: bool):
        ...
        mod.modelA.decoder.deep_supervision = enabled
    ...
    def train_step(self, batch: dict) -> dict:
        ...
        with autocast(self.device.type, enabled=True) if self.device.type == 'cuda' else dummy_context():
            ...
            #tatuanb
            import torchattacks
            #atk = torchattacks.FGSM(self.network, eps=0.1)
            atk = torchattacks.PGD(self.network, eps=0.1, alpha=0.05, steps=10)
            target2 = [torch.ones_like(tensor) for tensor in target]
            data2 = atk(data, target2, self.loss)
            output2 = self.network(data2)

            l = (self.loss(output, target) + self.loss(output2, target))/2.0
